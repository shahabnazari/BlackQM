...
function logMetric(metric, value, threshold) {
  ...
  console.log(`  ${chalk.gray('├─')} ${metric}: ${value}ms (threshold: ${threshold}ms) ${status}`);
}
...
class PipelineE2ETest {
  ...
  async testLiteratureSearch() {
    ...
    return await timeOperation('Literature Search', async () => {
      ...
      console.log(`  ${chalk.gray('├─')} Papers found: ${this.testData.papers.length}`);
      console.log(`  ${chalk.gray('└─')} Sources: ${response.data.sources.join(', ')}`);

      return response.data;
    }, 3500); // 3.5s threshold (adjusted from Day 10 audit)
  }
  ...
  async testThemeExtraction() {
    ...
    return await timeOperation('Theme Extraction', async () => {
      const paperIds = this.testData.papers.slice(0, 5).map(p => p.id);

      const response = await axios.post(`${API_BASE}/literature/extract-themes`, {
        paperIds,
      ...
      console.log(`  ${chalk.gray('├─')} Themes extracted: ${this.testData.themes.length}`);
      console.log(`  ${chalk.gray('├─')} Controversies found: ${response.data.controversies?.length || 0}`);
      console.log(`  ${chalk.gray('└─')} Confidence score: ${response.data.confidence || 'N/A'}`);
  ...
  async testGapAnalysis() {
    ...
    return await timeOperation('Gap Analysis', async () => {
      const response = await axios.post(`${API_BASE}/literature/analyze-gaps`, {
      ...
      console.log(`  ${chalk.gray('├─')} Gaps identified: ${response.data.gaps.length}`);
      console.log(`  ${chalk.gray('├─')} Priority gaps: ${response.data.gaps.filter(g => g.priority === 'high').length}`);
      console.log(`  ${chalk.gray('└─')} Opportunities: ${response.data.opportunities?.length || 0}`);
  ...
  async testStatementGeneration() {
    ...
    return await timeOperation('Statement Generation', async () => {
      const response = await axios.post(`${API_BASE}/ai/statements/generate`, {
        themes: this.testData.themes,
      ...
      console.log(`  ${chalk.gray('├─')} Statements generated: ${this.testData.statements.length}`);
      console.log(`  ${chalk.gray('├─')} With provenance: ${this.testData.statements.filter(s => s.provenance).length}`);
      console.log(`  ${chalk.gray('└─')} Bias-checked: ${response.data.biasCheckPassed ? 'Yes' : 'No'}`);
  ...
  async testStudyCreation() {
    ...
    return await timeOperation('Study Creation', async () => {
      ...
      console.log(`  ${chalk.gray('├─')} Literature links: ${response.data.basedOnPapers?.length || 0}`);
      console.log(`  ${chalk.gray('└─')} Status: ${response.data.status || 'draft'}`);
  ...
  async testDataCollection() {
    ...
    return await timeOperation('Data Collection', async () => {
      // Generate mock participant responses
      const participants = 30;
      const responses = [];

      for (let i = 0; i < participants; i++) {
        const participantResponses = this.testData.statements.map((stmt, idx) => ({
          statementId: stmt.id,
      ...
      const response = await axios.post(`${API_BASE}/studies/${this.testData.studyId}/responses/batch`, {
        responses,
        mode: 'test'
      });

      console.log(`  ${chalk.gray('├─')} Participants simulated: ${participants}`);
      console.log(`  ${chalk.gray('├─')} Responses collected: ${responses.length * this.testData.statements.length}`);
      console.log(`  ${chalk.gray('└─')} Data quality score: ${response.data.qualityScore || 'N/A'}`);
  ...
  async testAnalysis() {
    ...
    return await timeOperation('Q-Analysis', async () => {
      const response = await axios.post(`${API_BASE}/analysis/q-methodology`, {
      ...
      console.log(`  ${chalk.gray('├─')} Factors extracted: ${response.data.factors.length}`);
      console.log(`  ${chalk.gray('├─')} Variance explained: ${response.data.varianceExplained || 'N/A'}%`);
      console.log(`  ${chalk.gray('└─')} Distinguishing statements: ${response.data.distinguishingStatements?.length || 0}`);
  ...
  async testLiteratureComparison() {
    ...
    return await timeOperation('Literature Comparison', async () => {
      const response = await axios.post(`${API_BASE}/analysis/compare-to-literature`, {
        studyId: this.testData.studyId,
      ...
      console.log(`  ${chalk.gray('├─')} Confirmatory findings: ${response.data.findings.confirmatory?.length || 0}`);
      console.log(`  ${chalk.gray('├─')} Novel findings: ${response.data.findings.novel?.length || 0}`);
      console.log(`  ${chalk.gray('├─')} Contradictory findings: ${response.data.findings.contradictory?.length || 0}`);
      console.log(`  ${chalk.gray('└─')} Discussion points: ${response.data.discussionPoints?.length || 0}`);
  ...
  async testReportGeneration() {
    ...
    return await timeOperation('Report Generation', async () => {
      ...
      console.log(`  ${chalk.gray('├─')} Sections generated: ${response.data.sections?.length || 0}`);
      console.log(`  ${chalk.gray('├─')} Word count: ${response.data.wordCount || 'N/A'}`);
      console.log(`  ${chalk.gray('├─')} Citations: ${response.data.citationCount || 0}`);
      console.log(`  ${chalk.gray('└─')} Format: ${response.data.format || 'unknown'}`);
  ...
  async testKnowledgeGraphUpdate() {
    ...
    return await timeOperation('Knowledge Graph Update', async () => {
      const response = await axios.post(`${API_BASE}/knowledge/update-graph`, {
        studyId: this.testData.studyId,
      ...
      console.log(`  ${chalk.gray('├─')} Nodes created: ${response.data.nodes.length}`);
      console.log(`  ${chalk.gray('├─')} Edges created: ${response.data.edges?.length || 0}`);
      console.log(`  ${chalk.gray('├─')} Patterns identified: ${response.data.patterns?.length || 0}`);
      console.log(`  ${chalk.gray('└─')} Graph density: ${response.data.density || 'N/A'}`);
    ...
    const validations = [
      {
        ...
        check: () => {
          ...
          return this.testData.papers.length > 0 &&
      ...
      },
      {
        ...
        check: () => {
          ...
          return this.testData.statements.filter(s => s.provenance).length >
                 this.testData.statements.length * 0.8; // 80% should have provenance
        }
      },
      {
        ...
        check: () => {
          ...
          return this.testData.comparisonResults?.findings &&
  ...
  generatePerformanceReport() {
    ...
    const totalTime = performanceMetrics.pipelineSteps.reduce((sum, step) => sum + step.duration, 0);
    const avgTime = Math.round(totalTime / performanceMetrics.pipelineSteps.length);
    ...
    console.log(`Steps Executed: ${chalk.cyan(performanceMetrics.pipelineSteps.length)}`);
    console.log(`Failed Thresholds: ${failedSteps.length > 0 ? chalk.red(failedSteps.length) : chalk.green('0')}`);

    if (failedSteps.length > 0) {
      console.log(chalk.yellow('\nSteps Exceeding Threshold:'));
      failedSteps.forEach(step => {
        console.log(`  - ${step.name}: ${step.duration}ms (threshold: ${step.threshold}ms)`);
      });
    }

    // Performance grade
    const performanceScore = ((performanceMetrics.pipelineSteps.length - failedSteps.length) /
                             performanceMetrics.pipelineSteps.length) * 100;
    let grade = 'A+';
  ...
  async run() {
    ...
    try {
      ...
      if (validation.failed === 0 && performanceMetrics.errors.length === 0) {
        ...
        console.log(chalk.green('The complete research pipeline is functioning correctly.'));
        process.exit(0);
      } else {
        console.log(chalk.yellow.bold('\n⚠️ PIPELINE TEST COMPLETED WITH ISSUES'));
        console.log(chalk.yellow(`Validation failures: ${validation.failed}`));
        console.log(chalk.yellow(`Errors encountered: ${performanceMetrics.errors.length}`));
        process.exit(1);
      }
...
(async () => {
  ...
  if (!isBackendAvailable) {
    ...
    console.log(chalk.yellow('Please start the backend with: npm run start:dev'));
