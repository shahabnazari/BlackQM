...
class UltimateDevManagerV5 {
  ...
  createLockFiles() {
    ...
    fs.writeFileSync(this.processListFile, JSON.stringify({
      manager: process.pid,
      frontend: null,
      backend: null,
  ...
  async startBackend() {
    ...
    this.backendProcess.stdout.on('data', (data) => {
      ...
      if (output.includes('Nest application successfully started')) {
        this.log('   ✅ Backend started successfully (PID:', this.backendProcess.pid + ')');
      }
    });
    ...
    this.backendProcess.on('exit', (code, signal) => {
      ...
      if (!this.isShuttingDown) {
        ...
        if (!this.isRestartingBackend && this.backendFailures < this.maxRestartAttempts) {
  ...
  async startFrontend() {
    ...
    this.frontendProcess.stdout.on('data', (data) => {
      ...
      if (output.includes('Ready in') || output.includes('compiled successfully')) {
        this.log('   ✅ Frontend started successfully (PID:', this.frontendProcess.pid + ')');
      }
    });
    ...
    this.frontendProcess.on('exit', (code, signal) => {
      ...
      if (!this.isShuttingDown) {
        ...
        if (!this.isRestartingFrontend && this.frontendFailures < this.maxRestartAttempts) {
  ...
  startHealthMonitoring() {
    ...
    this.healthCheckInterval = setInterval(async () => {
      ...
      const backendOk = await this.checkHttpHealth('http://localhost:4000/api', 'Backend');
