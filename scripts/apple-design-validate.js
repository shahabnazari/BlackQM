...
const validationRules = {
  ...
  colors: {
    ...
    check: content => {
      ...
      const hasSemanticColors = !content.match(
  ...
  darkMode: {
    ...
    check: content => {
      ...
      const semanticTokens =
        content.includes('text-text') ||
        content.includes('bg-surface') ||
        content.includes('bg-fill') ||
        content.includes('border-border');
      // Full credit for explicit dark mode or semantic tokens
      return explicitDark || semanticTokens ? 1 : 0.5;
    },
