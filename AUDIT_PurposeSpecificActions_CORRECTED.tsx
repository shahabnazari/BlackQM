/**
 * Purpose-Specific Actions Component - STRICT AUDIT MODE CORRECTED
 * Phase 10.91 Day 7: Extracted from ThemeExtractionContainer
 *
 * **AUDIT FIXES APPLIED:**
 * - ✅ Fixed ALL `any` types (Issues #4-10)
 * - ✅ Memoized component with React.memo (Issue #14)
 * - ✅ Added proper TypeScript imports from @/components/literature
 * - ✅ Added explicit type exports for better maintainability
 *
 * **Purpose:**
...
// AUDIT FIX #4-10: Import proper types instead of using `any`
import type {
...
export interface PurposeSpecificActionsProps {
  ...
  /** Generated survey data - AUDIT FIX #4: Properly typed (was `any | null`) */
  ...
  /** Generated research questions - AUDIT FIX #5: Properly typed (was `any[]`) */
  researchQuestions: ResearchQuestionSuggestion[];

  /** Generate research questions handler */
  onGenerateQuestions: () => void;

  /** Select question handler - AUDIT FIX #6: Properly typed (was `any`) */
  onSelectQuestion: (question: ResearchQuestionSuggestion) => void;

  /** Operationalize question handler - AUDIT FIX #7: Properly typed (was `any`) */
...
// ============================================================================
// Main Component
// AUDIT FIX #14: Memoized with React.memo
// ============================================================================

export const PurposeSpecificActions = React.memo(function PurposeSpecificActions({
  extractionPurpose,
  hasThemes,
  hasSelection,
  selectedCount,
            ...
            <Card className="border border-indigo-200 hover:border-indigo-400 transition-colors">
              <CardContent className="p-4">
                ...
                <Button
                  ...
                  size="sm"
                  aria-label="Generate Q-methodology statements from extracted themes"
                >
            ...
            <Card className="border border-amber-200 hover:border-amber-400 transition-colors">
              <CardContent className="p-4">
                ...
                <Button
                  ...
                  size="sm"
                  aria-label={showSurveyPrimary
                    ? "Generate validated survey with psychometric scales"
                    : "Generate survey from selected themes"}
                >
                  {loadingSurvey ? (
                    <>
              ...
              <Card className="border border-blue-200 hover:border-blue-400 transition-colors">
                <CardContent className="p-4">
                  ...
                  <Button
                    ...
                    className="w-full"
                    size="sm"
                    aria-label="Generate AI-suggested research questions from selected themes"
                  >
                    {loadingQuestions ? (
                      <>
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              ...
              <Card className="border border-purple-200 hover:border-purple-400 transition-colors">
                <CardContent className="p-4">
                  ...
                  <Button
                    ...
                    size="sm"
                    aria-label="Generate testable hypotheses from selected themes"
                  >
                    {loadingHypotheses ? (
                      <>
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              ...
              <Card className="border border-green-200 hover:border-green-400 transition-colors">
                <CardContent className="p-4">
                  ...
                  <Button
                    ...
                    size="sm"
                    aria-label="Map selected themes to theoretical constructs"
                  >
                    {loadingConstructs ? (
                      <>
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
