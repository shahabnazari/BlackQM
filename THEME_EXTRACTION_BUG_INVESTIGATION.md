...
const sourcesWithContent = paperSources.filter(
...
if (coherence < minCoherence) {
  // 0.6 or 0.7 depending on purpose
...
if (validatedThemes.length === 0 && themes.length > 0) {
  ...
  themes.slice(0, 3).forEach((theme, idx) => {
    ...
    const distinctiveness =
      idx > 0 ? this.calculateDistinctiveness(theme, [themes[0]]) : 1.0;
    const evidenceQuality =
      theme.codes.filter(c => c.excerpts.length > 0).length /
      theme.codes.length;
    this.logger.warn(`   ${idx + 1}. "${theme.label}"`);
    this.logger.warn(
      `      - Sources: ${theme.sourceIds.length} (need ${minSources})`
    );
    this.logger.warn(
      `      - Coherence: ${coherence.toFixed(2)} (need ${minCoherence})`
    );
    this.logger.warn(
      `      - Distinctiveness: ${distinctiveness.toFixed(2)} (need 0.3)`
    );
    this.logger.warn(
      `      - Evidence Quality: ${evidenceQuality.toFixed(2)} (need 0.5)`
    );
  });
}
```
