...
async function testLiteratureSearch() {
  ...
  try {
    ...
    console.error('❌ Public search failed:', error.response?.data?.message || error.message);
  }
  ...
  try {
    ...
    const frontendResponse = await axios.post('http://localhost:4000/api/literature/search/public', {
    ...
    console.error('❌ Frontend-style search failed:', error.response?.data?.message || error.message);
  }
