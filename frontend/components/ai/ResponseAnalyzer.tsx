...
export function ResponseAnalyzer({
  ...
  const calculateBehavioralMetrics = (response: QSortResponse) => {
    ...
    const responseTimeConsistency = Math.max(
      0,
      100 - (Math.abs(completionTime - expectedTime) / expectedTime) * 100
    );
  ...
  const calculateLinguisticDiversity = (
    surveyAnswers: Record<string, any>
  ): number => {
    const textResponses = Object.values(surveyAnswers).filter(
  ...
  const calculateEngagementDepth = (response: QSortResponse): number => {
    ...
    if (response.surveyAnswers) {
      ...
      const avgLength =
        answers.reduce((sum: number, ans: any) => {
  ...
  const identifyBotFlags = (
    response: QSortResponse,
    metrics: any
  ): string[] => {
    const flags: string[] = [];
    ...
    if (
      ...
      response.completionTime < response.qsort.length * 1000
    ) {
  ...
  const calculateAnomalyScore = (
    ...
    const zScores = scores.map(score =>
      Math.abs((score - mean) / (stdDev || 1))
    );
    const anomalyScore =
      (zScores.reduce((a, b) => a + b, 0) / zScores.length) * 20;

    return Math.min(100, anomalyScore);
  };

  const calculateBotProbability = (
    metrics: any,
    flags: string[],
    anomalyScore: number
  ): number => {
  ...
  const runBotDetection = () => {
    ...
    const detectionResults = responses.map(response =>
      detectBotPatterns(response)
    );
    // Bot detection results are used locally in this function

    // Calculate overall statistics
    const botCount = detectionResults.filter(
      r => r.recommendation === 'bot'
    ).length;
    // const suspiciousCount = detectionResults.filter(r => r.recommendation === 'suspicious').length;
    const humanCount = detectionResults.filter(
      r => r.recommendation === 'human'
    ).length;
    ...
    setAnalysisResult(prev => ({
      ...
      botDetection: {
        ...
        suspiciousResponses: detectionResults.filter(
          r => r.recommendation === 'suspicious'
        ),
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await execute({
        ...
        analysisTypes: selectedAnalyses as Array<
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
                ...
                <input
                  ...
                  onChange={e => {
                    if (e.target.checked) {
                      ...
                      setSelectedAnalyses(
                        selectedAnalyses.filter(a => a !== type.value)
                      );
                    }
                  }}
                  className="h-4 w-4"
        ...
        <Button
          ...
          disabled={
            loading || responses.length === 0 || selectedAnalyses.length === 0
          }
          className="w-full"
        >
          {loading ? (
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                {analysisResult.quality && (
                  <div className="text-center p-3 border rounded">
                    ...
                    <div
                      className={`text-2xl font-bold ${getQualityColor(analysisResult.quality.overallScore)}`}
                    >
                ...
                {analysisResult.patterns?.clusters && (
                  <div className="text-center p-3 border rounded">
                    ...
                    <div className="text-2xl font-bold">
                      {analysisResult.patterns.clusters.length}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Clusters Found
                    </div>
            ...
            <TabsContent value="patterns" className="space-y-4">
              {analysisResult.patterns ? (
                <>
                  {/* Clusters */}
                  {analysisResult.patterns.clusters &&
                    analysisResult.patterns.clusters.length > 0 && (
                      <div className="space-y-2">
                  ...
                  {/* Trends */}
                  {analysisResult.patterns.trends &&
                    analysisResult.patterns.trends.length > 0 && (
                      <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span
                          className={`text-sm font-medium ${getQualityColor(analysisResult.quality.overallScore)}`}
                        >
                          {analysisResult.quality.overallScore}% -{' '}
                          {getQualityLabel(analysisResult.quality.overallScore)}
                        </span>
                      </div>
                      <Progress
                        value={analysisResult.quality.overallScore}
                        className="h-2"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress
                        value={analysisResult.quality.completenessScore}
                        className="h-2"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress
                        value={analysisResult.quality.consistencyScore}
                        className="h-2"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Progress
                        value={analysisResult.quality.engagementScore}
                        className="h-2"
                      />
                    </div>
                  </div>
                  ...
                  {analysisResult.insights.keyFindings &&
                    analysisResult.insights.keyFindings.length > 0 && (
                      <div className="space-y-2">
                        ...
                        {analysisResult.insights.keyFindings.map(
                          (finding, i) => (
                    ...
                    analysisResult.insights.recommendations.length > 0 && (
                      <Alert>
                        ...
                        <AlertDescription>
                          ...
                          <ul className="space-y-1">
                            {analysisResult.insights.recommendations.map(
                              (rec, i) => (
            ...
            <TabsContent value="botdetection" className="space-y-4">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    <div className="p-3 border rounded text-center">
                      ...
                      <div className="text-xl font-bold text-purple-600">
                        {Math.round(
                          analysisResult.botDetection.detectionConfidence
                        )}
                        %
                      </div>
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-2 text-xs">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span>
                                  Pattern:{' '}
                                  {Math.round(
                                    bot.behavioralMetrics.patternRegularity
                                  )}
                                  %
                                </span>
                              </div>
                              <div className="flex items-center gap-1">
                                ...
                                <span>
                                  Engagement:{' '}
                                  {Math.round(
                                    bot.behavioralMetrics.engagementDepth
                                  )}
                                  %
                                </span>
                              </div>
                            ...
                            {bot.flags.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {bot.flags.map((flag, idx) => (
                                  <span
                                    key={idx}
                                    className="text-xs px-2 py-1 bg-white text-red-600 rounded border border-red-200"
                                  >
                                    {flag}
                                  </span>
                                ))}
                  ...
                  {analysisResult.botDetection.suspiciousResponses.length >
                    ...
                    <div className="space-y-2">
                      ...
                      <Alert>
                        <AlertTriangle className="h-4 w-4" />
                        <AlertDescription>
                          These responses show some bot-like characteristics but
                          require manual review.
                        </AlertDescription>
                      </Alert>
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {analysisResult.botDetection.suspiciousResponses
                          .slice(0, 5)
                          .map(suspicious => (
                            <div
                              key={suspicious.participantId}
                              className="p-2 border border-yellow-200 rounded bg-yellow-50"
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-yellow-700">
                                    {Math.round(suspicious.botProbability)}% Bot
                                    Probability
                                  </span>
                                  <Button
                                    size="sm"
                    ...
                    <AlertDescription>
                      ...
                      <ul className="text-sm space-y-1">
                        {analysisResult.botDetection.overallBotRate > 10 && (
                          <li>
                            • Consider removing detected bot responses before
                            analysis
                          </li>
                        )}
                        {analysisResult.botDetection.suspiciousResponses
                          .length > 0 && (
                          <li>
                            • Manually review suspicious responses for final
                            determination
                          </li>
                        )}
                        <li>
                          • Use CAPTCHA or response time limits for future data
                          collection
                        </li>
                        <li>
                          • Implement honeypot questions to catch automated
                          responses
                        </li>
                      </ul>
                    </AlertDescription>
              ...
              {!analysisResult.botDetection && (
                <Alert>
                  <Info className="h-4 w-4" />
                  <AlertDescription>
                    Click "Run Detection" to analyze responses for bot patterns
