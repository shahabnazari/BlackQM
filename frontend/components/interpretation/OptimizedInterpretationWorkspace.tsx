...
const OptimizedInterpretationWorkspace = memo(
  ({
    ...
    const narrativeCompleteness = useMemo(() => {
      ...
      const completeCount = narratives.filter(
        n => n.narrative?.length > 100
      ).length;
    ...
    const tabContent = useMemo(() => {
      ...
      switch (activeTab) {
        ...
        case 'bias':
          return <BiasAnalysisPanel {...commonProps} />;
        case 'perspectives':
          return <PerspectiveValidator {...commonProps} statements={[]} />;
        case 'alternatives':
          return (
        ...
        case 'mining':
          return <QuoteMiner {...commonProps} data={{}} />;
        case 'patterns':
          return <QualitativePatternDetector {...commonProps} data={{}} />;
        case 'distinguishing':
          return (
            <DistinguishingViewAnalyzer {...commonProps} factors={factors} />
          );
        case 'interactions':
          return (
        ...
        case 'insights':
          return <InsightAggregator {...commonProps} insights={[]} />;
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Main Content Area */}
        <Card className="p-6 bg-white min-h-[600px]">{tabContent}</Card>
        ...
        {process.env.NODE_ENV === 'development' && (
          <Card className="p-4 bg-gray-50 text-xs text-gray-600">
            <div className="flex items-center justify-between">
              ...
              <span>
                Factors: {factorCount} | Narratives: {narratives?.length || 0} |
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.analysisResults?.factors?.length ===
        nextProps.analysisResults?.factors?.length
    );
  }
);

OptimizedInterpretationWorkspace.displayName =
