...
import { FileText, Video, Podcast, Share2, ExternalLink, Clock, Award, FileCheck } from 'lucide-react';
import type {
  UnifiedTheme,
  ThemeSource,
...
export const ThemeProvenancePanel: React.FC<ThemeProvenancePanelProps> = ({ theme, onClose }) => {
  ...
  const getSourceUrl = (source: ThemeSource): string => {
    ...
    if (source.sourceUrl) {
      ...
      if (source.sourceType === 'youtube' && source.timestamps && source.timestamps[0]) {
              ...
              {chartData.map((item) => (
                <div key={item.name} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {item.count} {item.count === 1 ? 'source' : 'sources'} • {item.value.toFixed(1)}% influence
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium text-gray-900 dark:text-white mb-1">
                    Confidence: {(theme.provenance.averageConfidence * 100).toFixed(1)}%
                  </div>
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
            {topSources.map((source, index) => (
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    {source.sourceAuthor && (
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        {source.sourceAuthor} {source.year && `(${source.year})`}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {source.timestamps && source.timestamps.length > 0 && source.timestamps[0] && (
                  <div className="mb-2 pl-12">
                    <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                      <Clock className="w-3 h-3" />
                      <span>Relevant at {formatTimestamp(source.timestamps[0].start)}</span>
                    </div>
                  </div>
                )}
      ...
      {theme.provenance.citationChain.length > 0 && (
        <div className="border-t border-gray-200 dark:border-gray-700 px-6 py-4">
          ...
          <div className="flex flex-wrap gap-2">
            {theme.provenance.citationChain.slice(0, 10).map((citation, index) => (
              <a
                key={index}
                href={citation.startsWith('http') ? citation : `https://doi.org/${citation}`}
                target="_blank"
                rel="noopener noreferrer"
