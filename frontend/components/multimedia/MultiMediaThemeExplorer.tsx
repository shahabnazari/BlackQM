  ...
  const sortedThemes = useMemo(() => {
    return Object.entries(themeGroups)
      .map(([themeName, themeInstances]) => ({
        ...
        totalRelevance: themeInstances.reduce(
          (sum, t) => sum + t.relevanceScore,
          0
        ),
        avgRelevance:
          themeInstances.reduce((sum, t) => sum + t.relevanceScore, 0) /
          themeInstances.length,
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="secondary">
                {themes.length} themes across {Object.keys(themeGroups).length}{' '}
                topics
              </Badge>
              {onGenerateStatements && (
                <Button
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <Badge variant="secondary">
                          {Math.round(themeGroup.avgRelevance * 100)}% avg
                          relevance
                        </Badge>
                        <Badge variant="outline">
                          {themeGroup.sourceCount} source
                          {themeGroup.sourceCount > 1 ? 's' : ''}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                ...
                <div className="space-y-2">
                  {timelineData.map((item, idx) => (
                    <div
                      ...
                      onClick={() =>
                        onTimestampClick?.(item.sourceTitle, item.start)
                      }
                    >
