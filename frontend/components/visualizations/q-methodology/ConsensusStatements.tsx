  ...
  return (
    ...
    >
      ...
      {consensusStatements.map((statement, i) => {
        ...
        const rangeWidth = xScale(statement.meanZScore + statement.standardDeviation) - xScale(statement.meanZScore - statement.standardDeviation);
        ...
        return (
          <Group key={statement.id}>
            ...
            <text
              ...
              onClick={() => setSelectedStatement(isSelected ? null : statement.id)}
            >
              {statement.id}. {statement.text.length > 35 
            ...
            <motion.rect
              ...
              style={{ cursor: 'pointer', transformOrigin: `${xScale(statement.meanZScore)}px ${yPos + barHeight / 2}px` }}
              onMouseEnter={() => {
                showTooltip({
            ...
            <motion.circle
              ...
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              transition={{ delay: i * 0.05 + 0.2, duration: 0.4, type: 'spring' }}
              style={{ cursor: 'pointer' }}
              onMouseEnter={() => {
                showTooltip({
                  tooltipData: statement,
...
&sigma;={statement.standardDeviation.toFixed(3)}
            ...
            <text
              ...
              textAnchor="middle"
            >
              {(statement.meanZScore - statement.standardDeviation).toFixed(1)}
            </text>
            <text
              x={xScale(statement.meanZScore + statement.standardDeviation)}
              y={yPos + barHeight / 2 + 25}
      ...
      <Group top={innerHeight - 60} left={innerWidth - 180}>
        <rect
          ...
          rx={8}
        />
        <text x={0} y={-10} fontSize={11} fontFamily="-apple-system" fontWeight="600" fill="#34C759">
          Consensus Summary
        </text>
        <text x={0} y={5} fontSize={10} fontFamily="-apple-system" fill="#666">
          Statements: {consensusStatements.length}
        </text>
        <text x={0} y={20} fontSize={10} fontFamily="-apple-system" fill="#666">
          Avg. Agreement: {(consensusStatements.reduce((sum, s) => sum + (1 - s.standardDeviation), 0) / consensusStatements.length).toFixed(2)}
        </text>
        <text x={0} y={35} fontSize={10} fontFamily="-apple-system" fill="#666">
    ...
    {tooltipOpen && tooltipData && (
      ...
      >
        <div style={{ fontFamily: '-apple-system', fontSize: '12px' }}>
          <div style={{ fontWeight: '600', color: '#34C759', marginBottom: '6px' }}>
            Statement {tooltipData.id}
          </div>
          <div style={{ marginBottom: '8px', fontSize: '11px', lineHeight: '1.4' }}>
            {tooltipData.text}
          </div>
          <div style={{ fontSize: '11px', color: '#666' }}>
            <div>Mean Z-Score: <span style={{ fontWeight: '600', color: '#34C759' }}>
              {tooltipData.meanZScore.toFixed(3)}
            </span></div>
            <div>Standard Deviation: <span style={{ fontWeight: '600' }}>
              {tooltipData.standardDeviation.toFixed(3)}
            </span></div>
            <div>Range: [{(tooltipData.meanZScore - tooltipData.standardDeviation).toFixed(2)}, {(tooltipData.meanZScore + tooltipData.standardDeviation).toFixed(2)}]</div>
          </div>
          {tooltipData.factorScores && (
            <div style={{ marginTop: '8px', fontSize: '10px', color: '#666' }}>
              <div style={{ fontWeight: '600', marginBottom: '4px' }}>Factor Scores:</div>
