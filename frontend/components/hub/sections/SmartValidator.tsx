...
export function SmartValidator({ 
  ...
  const [selectedTab, setSelectedTab] = useState<'statements' | 'responses' | 'statistical' | 'recommendations'>('statements');
    ...
    try {
      ...
      if (lastCheck) {
        checks[checks.length - 1] = {
          ...
          message: statementValidation.message || 'Statement validation complete',
      ...
      if (lastBiasCheck) {
        checks[checks.length - 1] = {
          ...
          status: biasAnalysis.analysis.overallBiasScore < 0.3 ? 'pass' : 
                  biasAnalysis.analysis.overallBiasScore < 0.6 ? 'warning' : 'fail',
      ...
      if (lastCulturalCheck) {
        checks[checks.length - 1] = {
          ...
          status: culturalCheck.issues?.length === 0 ? 'pass' : 
                  culturalCheck.issues?.length < 3 ? 'warning' : 'fail',
          message: culturalCheck.issues?.length === 0 ? 'No cultural issues detected' :
      ...
      if (lastIntegrityCheck) {
        checks[checks.length - 1] = {
          ...
          message: responseIntegrity.message || 'Response integrity check complete',
      ...
      const statisticalValidity = validateStatisticalRequirements(responses, statements);
      ...
      if (lastStatCheck) {
        checks[checks.length - 1] = {
          ...
          message: statisticalValidity.message || 'Statistical validity check complete',
      ...
      const qMethodCompliance = validateQMethodRequirements(responses, statements);
      ...
      if (lastQMethodCheck) {
        checks[checks.length - 1] = {
          ...
          message: qMethodCompliance.message || 'Q-methodology requirements check complete',
  ...
  const validateResponseIntegrity = (responses: any[]): Partial<ValidationCheck> => {
    const issues = [];
    ...
    const suspiciouslyFast = responses.filter(r => r.duration && r.duration < 60).length;
  ...
  const validateStatisticalRequirements = (responses: any[], statements: any[]): Partial<ValidationCheck> => {
    const issues = [];
    ...
    const expectedFactors = Math.floor(statements.length / minStatementsPerFactor);
    ...
    if (responses.length > 0) {
      ...
      const uniquePatterns = new Set(responses.map(r => JSON.stringify(r.rankings))).size;
    ...
    return {
      ...
      message: issues.length === 0 ? 'Statistical requirements met' : issues.join(', '),
      details: { issues, minResponses, expectedFactors },
    };
  };

  const validateQMethodRequirements = (responses: any[], statements: any[]): Partial<ValidationCheck> => {
    const issues = [];
    ...
    const unbalancedPerspectives = Object.values(statementsPerPerspective).some((count: any) => count < 5);
    ...
    return {
      status: issues.length === 0 ? 'pass' : issues.length < 2 ? 'warning' : 'fail',
      message: issues.length === 0 ? 'Q-methodology requirements satisfied' : issues.join(', '),
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {validationResults.statementValidation.details?.map((detail: any, idx: number) => (
            <Card key={idx}>
              <CardHeader>
    ...
    return (
      <div className="space-y-4">
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <Progress value={validationResults.biasAnalysis.overallBiasScore * 100} className="flex-1" />
              <span className="text-lg font-bold">
                {(validationResults.biasAnalysis.overallBiasScore * 100).toFixed(1)}%
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        {validationResults.biasAnalysis.biasedStatements?.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Biased Statements Detected</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {validationResults.biasAnalysis.biasedStatements.map((stmt: string, idx: number) => (
                  <div key={idx} className="p-2 bg-red-50 dark:bg-red-900/20 rounded">
                    ...
                    {validationResults.biasAnalysis.alternatives?.[stmt] && (
                      <p className="text-xs mt-1 text-green-600 dark:text-green-400">
                        Alternative: {validationResults.biasAnalysis.alternatives[stmt]}
                      </p>
                    )}
                  </div>
                ))}
      ...
      <div className="space-y-4">
        ...
        {validationResults.biasAnalysis?.recommendations && (
          <Card>
            ...
            <CardContent>
              <ul className="space-y-2">
                {validationResults.biasAnalysis.recommendations.map((rec: string, idx: number) => (
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            disabled={isValidating || (statements.length === 0 && responses.length === 0)}
            size="sm"
          >
            {isValidating ? (
              <>
        ...
        {validationResults && (
          <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-2">
              {overallScore >= 80 ? 'Excellent - Ready for analysis' :
               overallScore >= 60 ? 'Good - Minor issues detected' :
               overallScore >= 40 ? 'Fair - Several issues need attention' :
