...
const tourSteps: TourStep[] = [
  {
    ...
    description: 'We\'ve completely redesigned the navigation to make your research workflow smoother and more intuitive. Let\'s take a quick tour!',
    position: 'center',
    icon: Sparkles
  },
  {
    ...
    description: 'Your research journey is now organized into 10 clear phases: Discover, Design, Build, Recruit, Collect, Analyze, Visualize, Interpret, Report, and Archive.',
  ...
  },
  {
    ...
    description: 'On mobile devices, navigate using the bottom tabs and swipe between phases for a seamless experience.',
    position: 'center',
    icon: Smartphone,
    route: '/discover'
  },
  {
    ...
    description: 'Tablet users get a collapsible sidebar with all phases and quick settings access. Try it on your iPad!',
    position: 'center',
    icon: Tablet
  },
  {
    ...
    description: 'Press Cmd+K (or Ctrl+K on Windows) anytime to quickly jump to any feature. It\'s the fastest way to navigate!',
  ...
  },
  {
    ...
    description: 'Monitor your app\'s performance with real-time Core Web Vitals, bundle size analysis, and AI-powered recommendations.',
    position: 'center',
    icon: Gauge,
    route: '/performance'
  },
  {
    ...
    description: 'Swipe between phases, pull to refresh, and enjoy haptic feedback on supported devices. The app feels native!',
    position: 'center',
    icon: MousePointer
  },
  {
    ...
    description: 'New features are gradually rolled out using feature flags. You can preview experimental features in settings.',
    position: 'center',
    icon: Zap
  },
  {
    ...
    description: 'Ready to explore the new navigation? Start with the Discover phase to begin your research journey.',
    position: 'center',
    icon: Check,
    action: () => {
      confetti({
...
export function OnboardingTour({
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('start-navigation-onboarding', handleStartTour);
    };
  }, []);
  ...
  const handleComplete = () => {
    ...
    const prefs = JSON.parse(localStorage.getItem('new-navigation-preferences') || '{}');
      ...
      <div className="fixed inset-0 z-[100]">
        ...
        <motion.div
          ...
          className={cn(
            ...
            currentTourStep?.position === 'top' && "top-4 left-1/2 -translate-x-1/2",
            currentTourStep?.position === 'bottom' && "bottom-4 left-1/2 -translate-x-1/2",
            currentTourStep?.position === 'left' && "left-4 top-1/2 -translate-y-1/2",
            currentTourStep?.position === 'right' && "right-4 top-1/2 -translate-y-1/2",
            !currentTourStep?.position && highlightBox && "top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
          )}
          style={
