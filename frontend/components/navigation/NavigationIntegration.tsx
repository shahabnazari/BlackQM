...
import { useFeatureFlags as useMultipleFlags, FEATURE_FLAGS } from '@/lib/feature-flags';
import { useNavigationMigration, useRouteMigration, getCurrentPhase } from '@/lib/navigation/migration-helpers';
 ...
 * - Migration status
 */
export function NavigationIntegration({ children, className }: NavigationIntegrationProps) {
  // Feature flags
  ...
  const navigateToPhase = useCallback(async (phaseNumber: number) => {
    const phases = [
      '/discover', '/design', '/studies/create', '/recruitment',
      '/collect', '/analysis/hub', '/visualize', '/interpret',
      '/report', '/archive'
    ];
  ...
  return (
    <div className={cn("min-h-screen bg-background", className)}>
      ...
      {isMobile && (
        <>
          ...
          {flags[FEATURE_FLAGS.SWIPE_NAVIGATION] ? (
            ...
            <main className="min-h-screen pt-16 pb-20 p-4">
              {children}
            </main>
          )}
          
          {/* Mobile bottom navigation */}
          {flags[FEATURE_FLAGS.MOBILE_BOTTOM_TABS] && (
            <MobileNavigation />
          )}
        </>
      )}
      ...
      <AnimatePresence>
        ...
        <PhaseAvailability studyId="default" currentPhase={currentPhase || 'discover'} onPhaseSelect={() => {}} />
        
        {/* Onboarding Components */}
        {showTour && <TourComponent />}
        <PhaseOnboarding phase={currentPhase || 'discover'} studyId="default" userId="default" />
      </AnimatePresence>
      
      {/* Performance monitoring overlay (dev only) */}
      {process.env.NODE_ENV === 'development' && flags[FEATURE_FLAGS.REAL_TIME_METRICS] && (
        <div className="fixed bottom-4 left-4 z-50 bg-background/90 backdrop-blur-sm border rounded-lg p-2 text-xs">
          <div className="font-mono space-y-1">
            ...
            <div>Features: {Object.values(flags).filter(Boolean).length}/6</div>
          </div>
        </div>
      )}
    </div>
...
export { useFeatureFlag, useMultipleFlags as useFeatureFlags } from '@/lib/feature-flags';
export { getCurrentPhase, getNextPhase } from '@/lib/navigation/migration-helpers';
