...
export default function ParticipantPreview({ previewData, studyTitle }: ParticipantPreviewProps) {
  ...
  const renderStepContent = () => {
    ...
    switch (step.type) {
      case 'welcome':
        return (
          <div>
            ...
            {previewData.participantView.researcherBranding?.brandingPosition === 'top' && (
              <ResearcherBranding 
            ...
            <div className="space-y-4">
              <h2 className="text-3xl font-bold text-label">{(step.data as any).title}</h2>
              <div 
                className="prose prose-lg max-w-none text-secondary-label"
                dangerouslySetInnerHTML={{ __html: (step.data as any).message }}
              />
              {(step.data as any).includeVideo && (step.data as any).videoUrl && (
                <div className="aspect-video bg-black/5 rounded-lg flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-4xl mb-2">▶️</div>
                    <p className="text-sm text-secondary-label">Video Preview</p>
                    <p className="text-xs text-tertiary-label mt-1">{(step.data as any).videoUrl}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {previewData.participantView.researcherBranding?.brandingPosition === 'bottom' && (
              <ResearcherBranding 
                {...previewData.participantView.researcherBranding}
                position="bottom"
                variant="compact"
      ...
      case 'consent':
        return (
          <div>
            ...
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-label">Informed Consent</h2>
      ...
      case 'preScreening':
        return (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-label">Pre-Screening Questions</h2>
            <div className="space-y-4">
              {(step.data as any).questions.length > 0 ? (
                (step.data as any).questions.map((q: any, idx: number) => (
                  <div key={idx} className="p-4 border rounded-lg">
                    <p className="font-medium text-label mb-2">Question {idx + 1}</p>
              ...
              {(step.data as any).questions.length > 0 ? (
                ...
                <div className="p-8 text-center bg-quaternary-fill/10 rounded-lg">
                  ...
                  <p className="text-xs text-tertiary-label mt-2">
                    These help gather additional insights about your sorting decisions
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="bg-gray-100 px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="bg-white rounded-md px-3 py-1 text-xs text-gray-600 max-w-md mx-auto text-center">
              https://study.example.com/participate/{studyTitle.toLowerCase().replace(/\s+/g, '-')}
            </div>
          </div>
        </div>
