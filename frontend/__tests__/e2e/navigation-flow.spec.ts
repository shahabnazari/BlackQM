...
test.describe('Research Lifecycle Navigation Flow', () => {
  ...
  test('should display all 10 research phases in navigation', async ({ page }) => {
    // Check if all phases are present in the navigation
    for (const phase of PHASES) {
      const phaseElement = page.locator(`[data-phase="${phase.id}"]`).or(
        page.locator(`text=${phase.title}`).first()
      );
  ...
  test('should navigate through phases sequentially', async ({ page }) => {
    ...
    for (let i = 0; i < PHASES.length - 1; i++) {
      ...
      const nextButton = page.locator(`[data-next-phase="${nextPhase.id}"]`).or(
        page.locator(`a[href="${nextPhase.route}"]`).first()
      );
  ...
  test('should show phase progress indicators', async ({ page }) => {
    ...
    const progressIndicator = page.locator('[data-testid="phase-progress"]').or(
      page.locator('.phase-progress-indicator').first()
    );
  ...
  test('should handle phase prerequisites correctly', async ({ page }) => {
    ...
    const lockIndicator = page.locator('[data-locked="true"]').or(
      page.locator('.locked-phase').first()
    );
  ...
  test('should maintain navigation state across page refreshes', async ({ page }) => {
    ...
    const initialState = await page.evaluate(() => {
      return {
        ...
        phase: document.querySelector('[data-current-phase]')?.getAttribute('data-current-phase')
      };
    });
  ...
  test('should handle mobile navigation gestures', async ({ page, browserName }) => {
    ...
    const mobileNav = page.locator('[data-testid="mobile-nav"]').or(
      page.locator('.mobile-navigation').first()
    );
  ...
  test('should show AI-enhanced badges on relevant phases', async ({ page }) => {
    await page.goto(`${BASE_URL}/discover`);
    
    // Check for AI badges
    const aiBadges = page.locator('[data-ai-enhanced="true"]').or(
      page.locator('.ai-badge').first()
    );
  ...
  test('should handle route consolidation correctly', async ({ page }) => {
    ...
    for (const test of redirectTests) {
      ...
      await page.waitForURL(`**${test.new}**`, { timeout: 5000 }).catch(() => {});
...
test.describe('Phase-Specific Navigation Tests', () => {
  test('DISCOVER phase should have all sub-pages accessible', async ({ page }) => {
    const discoverPages = [
      '/discover/literature',
      '/discover/references',
  ...
  test('DESIGN phase should have methodology tools', async ({ page }) => {
    ...
    const methodologyTools = page.locator('[data-methodology]').or(
      page.locator('text=/methodology/i').first()
    );
  ...
  test('BUILD phase should integrate with study creation', async ({ page }) => {
    ...
    const studyBuilder = page.locator('[data-study-builder]').or(
      page.locator('text=/create.*study/i').first()
    );
  ...
  test('ANALYZE phase should show hub interface', async ({ page }) => {
    ...
    const analysisHub = page.locator('[data-analysis-hub]').or(
      page.locator('text=/analysis.*hub/i').first()
    );
