...
import {
  ...
  GeneratedQuestion,
  BiasAnalysis,
} from '../lib/services/ai-backend.service';

// Generic AI Hook with loading and error states
function useAIRequest<T, P>(serviceMethod: (params: P) => Promise<T>) {
...
export function useGenerateStatements() {
  return useAIRequest(
    aiBackendService.generateStatements.bind(aiBackendService)
  );
}
  ...
  const detectBias = useCallback(
    async (params: {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to detect bias';
        setError(errorMessage);
        return null;
      } finally {
        setLoading(false);
...
export function useParticipantAssistance() {
  return useAIRequest(
    aiBackendService.getParticipantAssistance.bind(aiBackendService)
  );
}
...
export function useAIBackend() {
  ...
  const generateStatements = useCallback(
    async (
      params: Parameters<typeof aiBackendService.generateStatements>[0]
    ) => {
      setLoading(true);
      setError(null);
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to generate statements';
        setError(message);
        throw err;
      } finally {
        setLoading(false);
  ...
  const getGridRecommendations = useCallback(
    async (
      params: Parameters<typeof aiBackendService.getGridRecommendations>[0]
    ) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const generateQuestionnaire = useCallback(
    async (
      params: Parameters<typeof aiBackendService.generateQuestionnaire>[0]
    ) => {
      setLoading(true);
      setError(null);
      try {
  ...
  // Detect Bias
  const detectBias = useCallback(
    async (params: Parameters<typeof aiBackendService.detectBias>[0]) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const analyzeResponses = useCallback(
    async (params: Parameters<typeof aiBackendService.analyzeResponses>[0]) => {
      setLoading(true);
      setError(null);
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to analyze responses';
        setError(message);
        throw err;
      } finally {
        setLoading(false);
  ...
  const getParticipantAssistance = useCallback(
    async (
      params: Parameters<typeof aiBackendService.getParticipantAssistance>[0]
    ) => {
      setLoading(true);
      setError(null);
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to get assistance';
        setError(message);
        throw err;
      } finally {
        setLoading(false);
  ...
  // Smart Validation
  const validateSmartly = useCallback(
    async (params: Parameters<typeof aiBackendService.validateSmartly>[0]) => {
      setLoading(true);
      setError(null);
      try {
...
export function useStatementVariations() {
  ...
  const generateVariations = useCallback(
    (statement: string, count: number = 3, debounce: number = 500) => {
      ...
      timeoutRef.current = setTimeout(async () => {
        ...
        try {
          const result = await aiBackendService.generateStatementVariations(
            statement,
            count
          );
