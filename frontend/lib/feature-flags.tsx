...
export function useMultipleFlags(flags: FeatureFlag[]): Record<FeatureFlag, boolean> {
  const isEnabled = useFeatureFlags((state) => state.isEnabled);
...
export function isFeatureEnabled(flag: FeatureFlag, userId?: string): boolean {
  ...
  if (config.rolloutPercentage !== undefined && config.rolloutPercentage < 100 && userId) {
...
export function FeatureFlagsDebugPanel() {
  const { flags, overrides, isEnabled, setOverride, clearOverride, clearAllOverrides, resetToDefaults } = useFeatureFlags();
    ...
    <div className="fixed bottom-4 right-4 z-50 bg-background border rounded-lg shadow-lg p-4 max-w-md max-h-96 overflow-auto">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="space-x-2">
          <button onClick={clearAllOverrides} className="text-xs px-2 py-1 bg-secondary rounded">
            Clear Overrides
          </button>
          <button onClick={resetToDefaults} className="text-xs px-2 py-1 bg-secondary rounded">
            Reset
          </button>
        </div>
      </div>
          ...
          return (
            <div key={flag} className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => hasOverride ? clearOverride(flag) : setOverride(flag, !enabled)}
