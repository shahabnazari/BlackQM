...
export function generateSampleConsensusStatements(
  ...
  for (let i = 0; i < Math.min(statementCount, consensusTexts.length); i++) {
    ...
    const meanZScore =
      factorScores.reduce((sum, s) => sum + s.score, 0) / factorScores.length;
    const standardDeviation = Math.sqrt(
      factorScores.reduce(
        (sum, s) => sum + Math.pow(s.score - meanZScore, 2),
        0
      ) / factorScores.length
    );
...
export function generateSampleQMethodologyData() {
  ...
  return {
    ...
    distinguishingStatements: generateSampleDistinguishingStatements(
      15,
      factors
    ),
