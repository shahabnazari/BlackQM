/**
 * Phase 10.93 Day 7 - Feature Flag Configuration (STRICT AUDIT CORRECTED)
 *
 * Centralized feature flag management for gradual rollout and safe rollback.
 *
 * **STRICT AUDIT FIXES:**
 * - ✅ BUG-001: Fixed client/server environment variable reading
 * - ✅ TYPE-001: Added proper window interface extension (no `any`)
 * - ✅ PERF-001: Optimized feature flag evaluation (module-level caching)
 *
 * **Usage:**
 * ```typescript
...
/**
 * Window extension for feature flag console helpers
 * STRICT AUDIT FIX: TYPE-001 - Proper interface instead of `any`
 */
declare global {
  interface Window {
    __featureFlags?: {
      get: () => FeatureFlagConfig;
      getMetadata: () => FeatureFlagMetadata[];
      set: (key: keyof FeatureFlagConfig, value: boolean) => void;
      clear: (key: keyof FeatureFlagConfig) => void;
      clearAll: () => void;
      help: () => void;
    };
  }
 ...
 * - Fixed client/server environment variable reading
 * - Client-side: Uses webpack-injected NEXT_PUBLIC_* vars (build-time)
 * - Server-side: Uses process.env (runtime)
 *
 * @param key - Environment variable name (must start with NEXT_PUBLIC_ for client access)
 * @param defaultValue - Default value if not set
 * @returns Boolean value
 */
function readEnvFlag(key: string, defaultValue: boolean): boolean {
  // STRICT AUDIT FIX: BUG-001
  // Next.js only exposes NEXT_PUBLIC_* variables to client-side code
  ...
  // Client-side: NEXT_PUBLIC_* vars are available via process.env (webpack injected)
  // IMPORTANT: These are build-time constants, not runtime variables
  const value = process.env[key];
  if (value === undefined) return defaultValue;
  return value === 'true' || value === '1';
}
 ...
 *
 * STRICT AUDIT NOTE: Module-level evaluation provides caching
 * If runtime changes are needed, use getFeatureFlags() function
 */
 ...
 * STRICT AUDIT FIX: TYPE-001 - Uses proper Window interface extension
 */
