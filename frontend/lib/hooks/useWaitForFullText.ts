  ...
  const fetchBulkStatus = useCallback(
    async (
      ...
      try {
        ...
        console.log(
          `🔍 [fetchBulkStatus] Calling ${url} with ${paperIds.length} paper IDs`
        );
        console.log(
          `🔍 [fetchBulkStatus] Paper IDs:`,
          paperIds.slice(0, 3).join(', ') + (paperIds.length > 3 ? '...' : '')
        );

        // Phase 10 Day 31.5: CRITICAL FIX - Add Authorization header for authentication
        const authHeaders = getAuthHeaders();
        console.log(
          `🔐 [fetchBulkStatus] Auth headers:`,
        ...
        console.log(
          `🔍 [fetchBulkStatus] Response status: ${response.status} ${response.statusText}`
        );
        ...
        console.log(
          `🔍 [fetchBulkStatus] Ready paper IDs:`,
          result.ready?.slice(0, 3).join(', ') +
            (result.ready?.length > 3 ? '...' : '')
        );
        console.log(
          `🔍 [fetchBulkStatus] Not fetched IDs:`,
          result.not_fetched?.slice(0, 3).join(', ') +
            (result.not_fetched?.length > 3 ? '...' : '')
        );

        return result;
      } catch (error) {
        console.error(
          '❌ [fetchBulkStatus] Error fetching bulk status:',
          error
        );
        return null;
      }
  ...
  const waitForFullText = useCallback(
    async (
      ...
      try {
        ...
        console.log(
          `🔍 [FullTextWait] Fetching initial status to detect unfetchable papers...`
        );
        ...
        if (initialStatus) {
          ...
          console.log(
            `   • Not fetched (no job queued - missing DOI/PMID/URL): ${initialNotFetched}`
          );
            ...
            return {
              ...
              status: {
                ...
                progressPercent: Math.round(
                  (initialReady / paperIds.length) * 100
                ),
                elapsedSeconds: 0,
              },
              reason: 'success',
        ...
        while (true) {
          ...
          if (elapsedSeconds >= maxWaitSeconds) {
            console.log(
              `⏱️ [FullTextWait] Timeout reached after ${elapsedSeconds}s`
            );
            ...
            if (finalStatus) {
              const finalResult: FullTextWaitStatus = {
                ...
                progressPercent: Math.round(
                  (finalStatus.ready.length / paperIds.length) * 100
                ),
                elapsedSeconds,
              };
          ...
          const status: FullTextWaitStatus = {
            ...
            progressPercent: Math.round(
              (bulkStatus.ready.length / paperIds.length) * 100
            ),
            elapsedSeconds,
          };
          ...
          const cannotFetchCount =
            status.notFetched.length + status.failed.length;
        ...
        toast.error(
          'Error waiting for full-text. Proceeding with available content...',
          {
            duration: TOAST_DURATION_IMPORTANT_MS,
          }
        );
