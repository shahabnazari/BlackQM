...
export function useThemeExtractionWorkflow(
  ...
  // ✅ ENHANCEMENT: AbortController for user cancellation support
  ...
  const handleExtractThemes = useCallback(async () => {
    // ✅ ENHANCEMENT: Create AbortController for cancellation support
    ...
    try {
      ...
      if (!user || !user.id) {
        console.error('❌ Authentication required for theme extraction');
        toast.error(
      ...
      // ✅ ENHANCEMENT: Check if operation was cancelled
      if (signal.aborted) {
    ...
    // ✅ CRITICAL FIX (Phase 10.92 Day 18 - Strict Audit):
    ...
    // ✅ FIX (CRITICAL-003): Use latestPapersRef instead of stale papers prop
    ...
    if (stalePapers.length > 0) {
      ...
      try {
        ...
        // ✅ FIX (CRITICAL-006): Properly typed error handling
    ...
    // ✅ ENHANCEMENT: Check for cancellation after metadata refresh
    if (signal.aborted) {
    ...
    const savePaperWithRetry = async (
      ...
      // ✅ FIX (CRITICAL-006): Properly typed payload
      const savePayload: {
        title: string;
        authors: string[];
        year?: number;
    ...
    // ✅ ENHANCEMENT: Collect full-text promises to wait for completion
    const fullTextPromises: Promise<void>[] = [];

    // ✅ ENHANCEMENT: Save papers in PARALLEL with concurrency limit for performance
    ...
    console.log(
      `\n⚡ Processing ${papersToSave.length} papers in parallel (max ${MAX_CONCURRENT_SAVES} concurrent)...`
    );

    // Process papers in batches for controlled parallelism
    for (let i = 0; i < papersToSave.length; i += MAX_CONCURRENT_SAVES) {
      // ✅ ENHANCEMENT: Check for cancellation before each batch
      if (signal.aborted) {
      ...
      console.log(
        `   📦 Batch ${batchNumber}/${totalBatches}: Processing ${batch.length} papers in parallel...`
      );
      ...
      for (const result of batchResults) {
        if (result.status === 'fulfilled') {
          ...
          if (saveResult.success) {
            ...
            // ✅ FIX (CRITICAL-002): Don't mutate paper object - preserve original ID
              ...
              .then(updatedPaper => {
                if (isMountedRef.current && !signal.aborted) {
                  ...
                  setPapers((prev: Paper[]) => {
                    ...
                    latestPapersRef.current = updated;
                    return updated;
                  });
            ...
            console.log(
              `   🔄 Full-text extraction started (Paper ID: ${dbPaperId.substring(0, 8)}...)`
            );
          } else {
            // Handle save failure
          ...
          failedCount++;
          console.error(`   ❌ Unexpected error processing paper:`, result.reason);
        }

        // ✅ ENHANCEMENT: Real-time progress tracking
    ...
    // ✅ ENHANCEMENT: WAIT for all full-text extractions with real-time progress
    if (fullTextPromises.length > 0) {
      // ✅ ENHANCEMENT: Check for cancellation
      if (signal.aborted) {
        console.log('❌ Full-text extraction cancelled by user');
        return;
      }

      console.log(
        `\n⏰ Waiting for ${fullTextPromises.length} full-text extraction${fullTextPromises.length === 1 ? '' : 's'} to complete...`
      );

      // ✅ ENHANCEMENT: Real-time progress tracking for full-text extraction
      ...
      const trackedPromises = fullTextPromises.map((promise) =>
        promise.finally(() => {
          ...
          console.log(
            `   📊 Progress: ${completedCount}/${total} full-text extractions complete (${percentage}%)`
          );
        })
      );
    ...
    // ✅ ENHANCEMENT: Check for cancellation before content analysis
    if (signal.aborted) {
    ...
    const paperSources: SourceContent[] = selectedPapersToAnalyze
      .map(p => {
        // ✅ FIXED (Phase 10.92 Day 2 + Audit): Use shared content type classification helper
        ...
        return {
          ...
          metadata: {
            ...
            contentType,
            // STRICT AUDIT FIX: Use actual content type, not hasFullText flag
            fullTextStatus:
              contentType === ContentType.FULL_TEXT
                ? ('success' as const)
                : ('failed' as const),
    ...
    const videoSources: SourceContent[] = transcribedVideos.map(video => {
      ...
      return {
        ...
        title: video.title,
        content,
        // ✅ FIX (CRITICAL-006): Properly typed theme mapping
    ...
    console.log(`\n📊 [${requestId}] FILTERING RESULTS:`);
    console.log(`   • Total papers selected: ${beforeFilter}`);
    console.log(`   • Papers WITH content (will be used): ${papersWithContent}`);
    console.log(`   • Papers WITHOUT content (skipped): ${papersSkipped}`);
    console.log(`   • Total videos: ${beforeFilterVideos}`);
    console.log(`   • Videos WITH content (will be used): ${videosWithContent}`);
    if (videosSkipped > 0) {
      console.warn(`   ⚠️  Videos WITHOUT content (skipped): ${videosSkipped}`);
    }
    console.log(`   • TOTAL sources for extraction: ${allSources.length}`);
    ...
    if (papersSkipped > 0) {
      ...
      if (skippedPercentage >= 50) {
        console.warn(
          `⚠️ [${requestId}] WARNING: ${papersSkipped}/${beforeFilter} papers (${skippedPercentage}%) have no content and will be skipped!`
        );
        toast.warning(
        ...
        console.warn(
          `⚠️ [${requestId}] ${papersSkipped} papers will be skipped (no content)`
        );
        toast.info(
    ...
    // ✅ FIXED (Phase 10.92 Day 2): Use ContentType enum instead of string literals
    ...
    console.log(`      • Videos WITH content (will be used): ${videosWithContent} ✅`);
    if (videosSkipped > 0) {
      console.log(`      • Videos WITHOUT content (skipped): ${videosSkipped} ❌`);
    }
    console.log(`      • TOTAL for extraction: ${allSources.length}`);
    console.log(`\n   📊 Content Type Breakdown (of papers with content):`);
