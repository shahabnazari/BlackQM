...
const defaultGridConfig: GridConfiguration = {
  ...
  distribution: 'bell',
  instructions:
    'Please sort the statements according to your level of agreement.',
};
    ...
    immer((set, get) => ({
      ...
      setGridColumns: columns =>
        set(state => {
          ...
          state.gridConfiguration.totalCells = columns.reduce(
      ...
      updateColumnCells: (columnIndex, cells) =>
        set(state => {
          if (state.gridConfiguration.columns[columnIndex]) {
            ...
            if (state.gridConfiguration.symmetry) {
              const mirrorIndex =
                state.gridConfiguration.columns.length - 1 - columnIndex;
              if (
                mirrorIndex !== columnIndex &&
                state.gridConfiguration.columns[mirrorIndex]
              ) {
            ...
            state.gridConfiguration.totalCells =
              state.gridConfiguration.columns.reduce(
                (sum, col) => sum + col.cells,
                0
              );
            state.isDirty = true;
      ...
      applyDistribution: (distribution: any) =>
        set(state => {
        ...
        try {
          for (const file of uploadQueue) {
            ...
            const response = await fetch(
              `/api/studies/${studyMetadata.id}/stimuli`,
              {
                method: 'POST',
                body: formData,
              }
      ...
      loadFromServer: async studyId => {
        ...
        try {
          ...
          // Load stimuli
          const stimuliResponse = await fetch(
      ...
      handleWebSocketMessage: message => {
        ...
        switch (type) {
          ...
          case 'stimulus:removed':
            set(state => {
              state.stimuli = state.stimuli.filter(
                (s: any) => s.id !== payload.id
              );
            });
            break;
