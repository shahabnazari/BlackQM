...
export class GridConfigurationService {
  ...
  static readonly STANDARD_CONFIGS: StandardGridConfig[] = [
    {
      ...
      description:
        'Simple, quick grid for novice participants or pilot studies',
      range: { min: -3, max: 3 },
      totalItems: 25,
  ...
  static getAIRecommendation(params: {
    ...
    if (
      params.participantExpertise === 'expert' &&
      params.complexityLevel === 'complex'
    ) {
      recommendedConfig = this.getConfigById('comprehensive-49')!;
      reasoning.push(
        'Expert participants can handle larger grids with more nuanced distinctions'
      );
      reasoning.push(
        'Complex topics benefit from 11-point scale (±5) for detailed discrimination'
      );
      confidence = 95;
    } else if (
      params.participantExpertise === 'general' &&
      params.timeConstraint === 'short'
    ) {
      recommendedConfig = this.getConfigById('beginner-25')!;
      reasoning.push(
        'General public with time constraints work best with simpler grids'
      );
      reasoning.push(
        '7-point scale (±3) provides sufficient discrimination without overwhelming'
      );
      confidence = 90;
    } else if (
      params.studyType === 'exploratory' &&
      params.participantCount > 30
    ) {
      recommendedConfig = this.getConfigById('extended-40')!;
      reasoning.push(
        'Exploratory studies benefit from more items to capture diverse perspectives'
      );
      reasoning.push(
        'Larger participant groups can handle 40 items for richer factor analysis'
      );
      confidence = 88;
    } else if (
      params.studyType === 'confirmatory' &&
      params.complexityLevel === 'moderate'
    ) {
      recommendedConfig = this.getConfigById('optimal-36')!;
      reasoning.push(
        'Confirmatory studies work well with the standard 36-item configuration'
      );
      reasoning.push(
        'This is the most validated configuration in Q-methodology literature'
      );
      confidence = 92;
    } else {
      ...
      recommendedConfig = this.getConfigById('optimal-36')!;
      reasoning.push(
        '36-item grid is the gold standard in Q-methodology (Brown, 1980)'
      );
      reasoning.push(
        '9-point scale (±4) balances discrimination with cognitive load'
      );
      confidence = 85;
    }
    ...
    if (
      params.timeConstraint === 'short' &&
      recommendedConfig.totalItems > 36
    ) {
    ...
    if (
      params.previousExperience === 'none' &&
      recommendedConfig.totalItems > 40
    ) {
      recommendedConfig = this.getConfigById('optimal-36')!;
      reasoning.push(
        'Standard configuration recommended for first-time researchers'
      );
      confidence -= 3;
    }

    // Get alternatives
    const alternatives = this.STANDARD_CONFIGS.filter(
      (c: any) => c.id !== recommendedConfig.id
    )
      .filter((c: any) => {
        // Filter based on constraints
        if (params.timeConstraint === 'short' && c.totalItems > 36)
          return false;
        if (
          params.participantExpertise === 'general' &&
          c.expertiseLevel === 'expert'
        )
          return false;
        return true;
      })
    ...
    reasoning.push(
      `Based on ${recommendedConfig.citation} research validation`
    );
  ...
  static getConfigurationRationale(config: StandardGridConfig): string[] {
    ...
    rationale.push(
      `${rangeSize}-point scale provides ${
        rangeSize <= 7
          ? 'simple discrimination suitable for quick sorting'
          : rangeSize <= 9
            ? 'balanced discrimination without cognitive overload'
            : rangeSize <= 11
              ? 'detailed discrimination for nuanced perspectives'
    ...
    rationale.push(
      `${config.totalItems} items ${
        config.totalItems <= 30
          ? 'minimize participant fatigue while maintaining validity'
          : config.totalItems <= 40
            ? 'provide optimal balance of detail and completion time'
            : config.totalItems <= 50
              ? 'enable comprehensive factor analysis'
    ...
    rationale.push(
      `Distribution with maximum ${maxHeight} items per column follows quasi-normal distribution validated by ${config.citation}`
    );
