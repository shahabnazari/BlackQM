...
export class AIService {
  ...
  async generateCompletion(request: AIRequest): Promise<AIResponse> {
    if (!this.isInitialized) {
      throw this.createError('NOT_INITIALIZED', 'AI Service not initialized', false);
    }
    
    // Check budget
    const budget = this.costTracker.getBudget();
    if (budget.remaining <= 0) {
      throw this.createError('BUDGET_EXCEEDED', 'Daily AI budget exceeded', false);
    }
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            throw this.createError('UNAUTHORIZED', 'Authentication required', false);
          }
