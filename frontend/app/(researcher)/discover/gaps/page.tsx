...
export default function ResearchGapsPage() {
  ...
  useEffect(() => {
    async function fetchResearchGaps() {
      try {
        ...
        const scoredResult =
          await literatureAPI.scoreResearchOpportunities(gapIds);

        console.log(
          `💎 Scored ${scoredResult.opportunities.length} opportunities`
        );

        // Step 4: Transform backend data to frontend format
        const transformedGaps: ResearchGap[] = scoredResult.opportunities.map(
          (opp: any) => {
            const originalGap = analyzedGaps.find(
              (g: any) => g.id === opp.gapId
            );
            ...
            return {
              ...
              importance:
                opp.opportunityScore > 0.8
                  ? 'critical'
                  : opp.opportunityScore > 0.6
                    ? 'high'
                    : opp.opportunityScore > 0.4
                      ? 'medium'
                      : 'low',
              feasibility:
                opp.scoringFactors.feasibility > 0.7
                  ? 'high'
                  : opp.scoringFactors.feasibility > 0.5
                    ? 'medium'
                    : 'low',
              ...
              requiredExpertise: opp.suggestedCollaborators.map(
                (c: any) => c.expertise[0]
              ),
              potentialChallenges: [],
              opportunityScore: Math.round(opp.opportunityScore * 100),
              trend:
                opp.scoringFactors.timeliness > 0.7
                  ? 'emerging'
                  : opp.scoringFactors.timeliness > 0.3
                    ? 'stable'
                    : 'declining',
        ...
        console.log(
          `✅ Loaded ${transformedGaps.length} research gaps successfully`
        );
      } catch (err: any) {
  ...
  const loadFundingProbability = async (gapId: string) => {
    try {
      ...
      const opportunity =
        result.fundingOpportunities?.[0] || result.highProbability?.[0];
      setFundingProbability(opportunity);
      console.log(
        `✓ Loaded funding probability: ${opportunity ? (opportunity.probability * 100).toFixed(1) : 'N/A'}%`
      );
    } catch (err: any) {
  ...
  const loadImpactPrediction = async (gapId: string) => {
    try {
      ...
      const prediction =
        result.predictions?.[0] || result.transformativeOpportunities?.[0];
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    Using AI to identify opportunities in your literature
                  </p>
                </div>
              </div>
            </CardContent>
                                    ...
                                    <Card className="border-green-200 bg-green-50">
                                      <CardContent className="p-4">
                                        <h4 className="font-semibold text-green-900 mb-2 flex items-center gap-2">
                                          ...
                                          Funding Probability:{' '}
                                          {(
                                            fundingProbability.probability * 100
                                          ).toFixed(1)}
                                          %
                                        </h4>
                                        <div className="space-y-2 text-sm">
                                          {fundingProbability.matchedGrants &&
                                            fundingProbability.matchedGrants
                                              .length > 0 && (
                                              <div>
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                  {fundingProbability.matchedGrants.map(
                                                    (
                                                      grant: any,
                                                      idx: number
                                                    ) => (
                                        ...
                                        <div className="space-y-2 text-sm">
                                          ...
                                          {timelineOptimization.phases && (
                                            <div>
                                              ...
                                              {timelineOptimization.phases.map(
                                    ...
                                    <Card className="border-purple-200 bg-purple-50">
                                      <CardContent className="p-4">
                                        ...
                                        <div className="space-y-2 text-sm">
                                          <p className="text-gray-700">
                                            <span className="font-medium">
                                              Predicted Citations (Year 5):
                                            </span>{' '}
                                            {impactPrediction.predictedCitations ||
                                              'N/A'}
                                          </p>
                                          <Progress
                                            value={Math.min(
