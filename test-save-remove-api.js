...
async function testSaveRemove() {
  try {
    ...
    let savedPaperId;
    try {
      const saveRes = await axios.post(`${API_BASE}/literature/save/public`, testPaper);
      console.log('✅ Save successful:', saveRes.data);
      savedPaperId = saveRes.data.paperId || fullPaper.id;
    } catch (error) {
      console.error('❌ Save failed:', error.response?.status, error.response?.data || error.message);
    ...
    try {
      const libraryRes = await axios.get(`${API_BASE}/literature/library/public`);
      ...
      const found = libraryRes.data.papers.find(p => p.title === testPaper.title);
      if (found) {
        console.log('✅ Test paper found in library');
        savedPaperId = found.id; // Update with actual ID from database
      }
    } catch (error) {
      console.error('❌ Library retrieval failed:', error.response?.status, error.response?.data || error.message);
    }
    ...
    if (savedPaperId) {
      try {
        const removeRes = await axios.delete(`${API_BASE}/literature/library/public/${savedPaperId}`);
        console.log('✅ Remove successful:', removeRes.data);
      } catch (error) {
        console.error('❌ Remove failed:', error.response?.status, error.response?.data || error.message);
      }
    } else {
    ...
    try {
      ...
      console.error('❌ Library verification failed:', error.response?.status, error.response?.data || error.message);
    }
