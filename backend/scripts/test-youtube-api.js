...
console.log(
  `API Key: ${YOUTUBE_API_KEY ? `${YOUTUBE_API_KEY.substring(0, 20)}...` : 'NOT FOUND'}`,
);
    ...
    res.on('end', () => {
      try {
        ...
        if (response.error) {
          ...
          console.error(
            `   Details: ${JSON.stringify(response.error.errors, null, 2)}`,
          );
          process.exit(1);
        }
        ...
        console.log(
          `✅ SUCCESS! Retrieved ${response.items.length} real YouTube videos:\n`,
        );
