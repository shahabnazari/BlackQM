...
export class AIMonitoringService {
  ...
  async trackUsage(data: {
    ...
    try {
      ...
      if (data.cost > 0.5) {
        this.logger.warn(
          `High-cost AI request: User ${data.userId}, Cost: $${data.cost.toFixed(4)}, Model: ${data.model}`,
        );
      }
    } catch (error) {
  ...
  async checkBudgetLimits(userId: string): Promise<UserUsage> {
    ...
    const threshold = Number(budgetLimit?.alertThreshold || 0.8);
    ...
    if (usage.isOverLimit) {
      ...
      this.logger.error(
        `User ${userId} exceeded daily AI budget: $${dailyTotal.toFixed(2)}/$${dailyLimit}`,
      );
    } else if (usage.isApproachingLimit) {
      this.eventEmitter.emit('ai.budget.warning', { userId, usage });
      this.logger.warn(
        `User ${userId} approaching daily AI budget: $${dailyTotal.toFixed(2)}/$${dailyLimit}`,
      );
    }
  ...
  async getMetrics(
    ...
    const averageResponseTime =
      usageRecords.reduce((sum, r) => sum + (r.responseTimeMs || 0), 0) /
        totalRequests || 0;
    ...
    const cacheHitRate =
      (Number(cacheHits._sum.hitCount || 0) / totalRequests) * 100 || 0;
    ...
    const hourlyDistribution = new Array(24).fill(0).map((_, hour) => {
      const count = usageRecords.filter(
        (r) => new Date(r.createdAt).getHours() === hour,
      ).length;
  ...
  async cleanupCache(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Cleaned up ${rateLimitResult.count} old rate limit records`,
      );
    } catch (error) {
  ...
  async getRealTimeStats(): Promise<any> {
    ...
    const health = {
      ...
      averageResponseTime:
        recentActivity.reduce((sum, r) => sum + (r.responseTimeMs || 0), 0) /
          recentActivity.length || 0,
      errorRate:
        (recentActivity.filter((r) => r.status === 'error').length /
