...
export class VisualizationService {
  ...
  async generateCorrelationHeatmap(
    ...
    try {
      ...
      const result = await this.convertChart(
        svgNode.outerHTML,
        format,
        { width, height },
      );
  ...
  async generateFactorLoadingPlot(
    ...
    try {
      ...
      // Plot points
      const points = loadingData.filter((d: any) => 
      ...
      this.logger.error(`Failed to generate factor loading plot: ${error.message}`);
      throw new BadRequestException('Failed to generate visualization');
    }
  }
  ...
  async generateScreePlot(
    ...
    try {
      ...
      // Draw line
      const line = d3.line()
        .x((d: any, i: number) => (xScale(`F${i + 1}`) || 0) + xScale.bandwidth() / 2)
        .y((d: any) => yScale(d));
  ...
  async generateFactorArrays(
    ...
    const cacheKey = this.getCacheKey('factor-array', studyId, { factorNumber, ...options });
    const cached = await this.cacheService.get(cacheKey);
    if (cached) return cached;
  ...
  private async getCorrelationData(studyId: string) {
    ...
    return {
      participants: study?.responses?.map((r: any) => r.participantIdentifier) || [],
  ...
  private async getFactorArrayData(studyId: string, factorNumber: number) {
    ...
    return (analysisResults?.results as any)?.factorArrays?.[factorNumber - 1] || null;
  }
}