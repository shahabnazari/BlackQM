...
const TEST_PAPERS = [
  ...
  },
  {
    ...
    abstract:
      'Effective climate communication is essential for mobilizing adaptation action. Our experimental study tests different message frames across 2,000 participants. Results show that emphasizing local co-benefits increases support for adaptation by 45% compared to global risk framing. Visual storytelling and personal narratives prove more effective than scientific data alone. We provide evidence-based guidelines for climate communicators.',
    doi: '10.1000/test7',
  ...
  },
  {
    ...
    abstract:
      'Climate adaptation requires planning horizons extending to 2050 and beyond. We analyze 20 long-range adaptation plans identifying best practices in scenario planning, pathway approaches, and decision making under uncertainty. Flexible, adaptive strategies that can be adjusted as conditions change outperform rigid long-term commitments. Integration of adaptation planning with comprehensive and capital improvement plans ensures sustained implementation.',
    doi: '10.1000/test11',
...
async function testThemeExtraction(testCase: (typeof TEST_CASES)[0]) {
  ...
  console.log(
    `   Expected themes: ${testCase.expectedMin}-${testCase.expectedMax}`,
  );
  console.log(
    `   Average abstract length: ${Math.round(sources.reduce((sum, s) => sum + s.content.length, 0) / sources.length)} chars`,
  );
  ...
  try {
    ...
    if (response.data.themes && response.data.themes.length > 0) {
      ...
      if (response.data.themes.length > 10) {
        console.log(
          `   ... and ${response.data.themes.length - 10} more themes`,
        );
      }
    } else {
    ...
    if (response.data.saturationData) {
      console.log(`\n📈 Saturation Analysis:`);
      console.log(
        `   Saturation reached: ${response.data.saturationData.saturationReached ? 'Yes' : 'No'}`,
      );
      console.log(
        `   Recommended sources: ${response.data.saturationData.recommendedSourceCount || 'N/A'}`,
      );
    }
    ...
    if (response.data.methodology) {
      ...
      console.log(
        `   AI Role: ${response.data.methodology.aiRole?.substring(0, 100)}...`,
      );
    }
...
async function main() {
  ...
  console.log(
    `Test Papers: ${TEST_PAPERS.length} papers with climate adaptation theme`,
  );
  ...
  results.forEach((result, idx) => {
    ...
    if (result.success) {
      ...
      if (result.themeCount < testCase.expectedMin) {
        ...
        console.log(
          `   ✓ Within expected range (${testCase.expectedMin}-${testCase.expectedMax})`,
        );
      }
    } else {
  ...
  console.log('\n' + '='.repeat(80));
  console.log(
    `Tests completed: ${results.filter((r) => r.success).length}/${results.length} passed`,
  );
  console.log('='.repeat(80) + '\n');
}
