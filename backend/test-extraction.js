...
async function testExtraction() {
  try {
    ...
    const testSources = [
      {
        ...
        content:
          'This paper explores the impact of climate change on coastal communities. Our research shows that rising sea levels pose significant threats to infrastructure and livelihoods. We conducted a comprehensive survey of 500 households across three coastal regions. The results indicate increasing vulnerability and the need for adaptive strategies. Community resilience was identified as a key factor in successful adaptation.',
    ...
    console.log(
      '🎯 Themes extracted:',
      extractResponse.data.themes?.length || 0,
    );
    ...
    if (extractResponse.data.themes && extractResponse.data.themes.length > 0) {
      ...
      console.log(
        'Full response:',
        JSON.stringify(extractResponse.data, null, 2),
      );
    }
  } catch (error) {
